...
from arch.PixelShuffle1D import SpatialShuffle_plus
import numpy as  np


class get_edge_feature2(nn.Module):
    ...
    def __init__(self, k=16):
        super(get_edge_feature2, self).__init__()
    ...
    def forward(self, point_cloud,input):
        ...
        '''
        idx = idx[:, 1:, :]
        tmp = input.view(B,3*C,N)
        point_cloud_neighbors = grouping_operation(tmp, idx.contiguous().int())
        point_cloud_neighbors = point_cloud_neighbors.view(B,C,self.k,3,N)
...
class resconv(nn.Module):
    def __init__(self, in_channels=64,k=16):
        ...
        self.edge_feature_model = get_edge_feature2(k=k)
        self.k = k
        self.in_channels = in_channels*2
        self.expansionrate = 2
        '''
