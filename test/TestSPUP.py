...
from Upsample.PUF3D4x2ssup import Generator
...
def infer(model, G_model, test_loader, criterion, opt):
    ...
    with torch.no_grad():
        for i, (data, gt, label) in enumerate(tqdm(test_loader)):
            ...
            preds = G_model(data)  # points.shape[1])
            # classification
...
if __name__ == '__main__':
    ...
    # PUF3D_Mid8x_ssup.parm PUF3D_Pre4_ssup2
    G_model.load_state_dict(torch.load('../Upsample/savedModel/PUF3D_Pre4_ssup2.parm'))
    G_model.cuda()
