...
sys.path.append('../')
import torch
from Upsample.PUF3D16xfspnoscaling import Generator
...
def train(args):
    ...
    ########### load dataset
    num_workers = 4
    trainloader = ModelNet40spu(data_dir=params["dataset_dir"], partition='train', num_points=params["num_points"],upscalefactor=16)
    train_data_loader = data.DataLoader(dataset=trainloader, batch_size=params["batch_size"], shuffle=True,
                                        num_workers=num_workers, pin_memory=True, drop_last=True)

    eval_dst = ModelNet40spu(data_dir=params["dataset_dir"], partition='test', num_points=params["num_points"],upscalefactor=16)
    ...
    for e in range(120):
        ...
        for batch_id, (input_data, gt_data,gtlabel) in enumerate(tqdmtrain):
            ...
            tqdmtrain.set_description(desc='epoch%d  total_G_loss:%f  loss_reg:%f' % (e, total_G_loss.item(),clsloss.item()))
            traincount+=1
        ...
        if Bestacc < top1.avg:
            Bestacc = top1.avg
            torch.save(G_model.state_dict(), './savedModel/PUF3D_Mid16x_fspnoscaling.parm')
    ...
    # 0.778
        ...
        data_frame.to_csv(out_path + 'PUF3D_Mid16x_fspnoscaling' + '.csv', index_label='Epoch')
