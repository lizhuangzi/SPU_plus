...
from arch.PixelShuffle1D import SpatialShuffle,SpatialShuffle_firstordernoscaling
import numpy as  np
...
class feature_extraction(nn.Module):
    ...
    def forward(self, x):
        B, D, N = x.size()
        #x = x.unsqueeze(dim=1)
        f0 = self.conv1(x)  # b,64,n
        f0 = self.conv2(f0)
        f0 = f0.unsqueeze(dim=1)
        f0 = f0.view(B,self.reschannel,3,N)
...
class SEPS(nn.Module):
    def __init__(self,inchannel,outchannel = 256,up_ratio=4):
        ...
        self.convspe4 = Conv2d(in_channels=outchannel, out_channels=2 * outchannel, kernel_size=1)
        self.upshuffle = SpatialShuffle_firstordernoscaling(self.up_ratio // 2, inchannel=2 * outchannel)
...
class Generator(nn.Module):
    ...
    def forward(self, input):
        ...
        return coord + nn.functional.interpolate(input=input,scale_factor=16,mode='nearest')
