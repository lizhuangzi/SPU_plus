...
from arch.PixelShuffle1D import SpatialShuffle,SpatialShuffle_plus
import numpy as  np
...
class resconv(nn.Module):
    def __init__(self, in_channels=64,k=16):
        ...
        self.conv1 = nn.Sequential(
            Conv3d(in_channels=self.in_channels, out_channels=self.in_channels*self.expansionrate, kernel_size=[1, 3,1], padding=[0, 1,0]),
            nn.ReLU()
        )
        self.conv2 = nn.Sequential(
            Conv3d(in_channels=self.in_channels*self.expansionrate, out_channels=self.in_channels*self.expansionrate, kernel_size=[1, 3,1], padding=[0, 1,0]),
            nn.ReLU()
        )
        self.conv3 = nn.Sequential(
...
class feature_extraction(nn.Module):
    def __init__(self,inchannel=56):
        super(feature_extraction, self).__init__()
        self.reschannel = inchannel

        self.conv1 = nn.Sequential(
            Conv2d(in_channels=1, out_channels=self.reschannel, kernel_size=1, padding=0),
            nn.ReLU(inplace=True)
        )

        self.res1conv = resconv(in_channels=self.reschannel)
        self.res1conv2 = resconv(in_channels=self.reschannel)
...
class SEPS(nn.Module):
    def __init__(self,inchannel,outchannel = 256,up_ratio=4):
        ...
        self.upshuffle = SpatialShuffle_plus(self.up_ratio // 2, inchannel=2 * outchannel)
...
class Generator(nn.Module):
    ...
    def forward(self, input):
        ...
        return coord + nn.functional.interpolate(input=input,scale_factor=4,mode='nearest')
